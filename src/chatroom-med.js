...
const ChatroomMed = (handleCurrentPageChange) => {
    // 跟頁面跳轉有關
    const location = useLocation(); // 使用 useLocation 钩子获取当前页面的路径
    // 检查 location.state 中的来源信息
    const isFromSchedule = location.state && location.state.from === 'schedulemed';
    // 根据来源信息设置返回链接的目标
    const backLinkTarget = isFromSchedule ? '/schedulemed' : '/homepagemed';
    if(isFromSchedule){
        var client_id = location.state.client_id;
        sessionStorage.setItem('client_id',client_id)
    }
    // 跟資料庫資料有關
    const sender_id = sessionStorage.getItem('user_id')
    const receiver_id = sessionStorage.getItem('client_id')
    
    // 跟聊天室顯示對方名子有關
    const [clientName , setClientName] = useState('');
    // const professional_id = sessionStorage.getItem('med_id'); 
    useEffect(() => {
        // 使用 Axios 透過professional_id向後端取得名字
        axios.get(process.env.REACT_APP_API_ADDRESS+`/get-user-backend-id/${client_id}`)
            .then(response => {
            // 從回應中取得spec並設定到狀態中
            setClientName(response.data[0].username); 
    ...
    useEffect(() => {
        axios.get(process.env.REACT_APP_API_ADDRESS+`/get-messages/${sender_id}/${receiver_id}`)
        .then((response) => {
        ...
        const htmlElements = pastMessages.map((message, index) => (
            <div key={index} className={message.sender_id == sender_id ? 'chat-item item-right clearfix' : 'chat-item item-left clearfix rela'}>
                <span className="abs uname">
                    ...
                    {message.sender_id == sender_id ? 'me' : 'Client'}
                </span>
